/**
 * @fileOverview Módulo que gestiona las funciones vitales en la construccion del laberinto
 * @module BaseDatos
...
function generaMatriz(dim, tam, esp) {
    let primero = new nodo.Nodo(tam, esp, esp);
    ...
    function buildRow(raiz, newO, newS, x, y) {
        let node = new nodo.Nodo(tam, x + tam, y);
        node.oeste = newO;
        newO.este = node;
        node.sur = newS;
        newS.norte = node;
    ...
    function buildColumn(raiz, newS, x, y) {
        let aux = new nodo.Nodo(tam, 0, y - tam);
        let node = new nodo.Nodo(tam, tam, y - tam);
        raiz.norte = aux;
        aux.sur = raiz;
        node.oeste = aux;
        aux.este = node;
